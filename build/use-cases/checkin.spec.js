...
var InMemoryGymsRepository = class {
  ...
  async create(data) {
    ...
    this.items.push(gym2);
    return gym2;
  }
  async findById(id) {
    const gym2 = this.items.find((item) => item.id === id);
    if (!gym2) {
      return null;
    }
    return gym2;
...
var CheckInUseCase = class {
  constructor(checkinsRepository, gymsRepository2) {
    this.checkinsRepository = checkinsRepository;
    this.gymsRepository = gymsRepository2;
  }
  async execute({ userId, gymId, userLatitude, userLongitude }) {
    const gym2 = await this.gymsRepository.findById(gymId);
    if (!gym2) {
      throw new ResourceNotFoundError();
    }
    const distance = getDistanceBetweenCoordinates(
      { latitude: userLatitude, longitude: userLongitude },
      { latitude: Number(gym2.latitude), longitude: Number(gym2.longitude) }
    );
