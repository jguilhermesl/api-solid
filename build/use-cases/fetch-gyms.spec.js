...
var InMemoryGymsRepository = class {
  ...
  async create(data) {
    ...
    const gym = {
      ...
      name: data.name,
      description: (_a2 = data.description) != null ? _a2 : null,
      phone: data.phone,
...
function createSuiteCollector(name, factory = () => {
  ...
  const test22 = createTest(function(name2, fn2 = noop, options = {}) {
    ...
    const test3 = task(
      formatName(name2),
      __spreadProps(__spreadValues(__spreadValues({}, this), options), { handler: fn2 })
    );
    test3.type = "test";
  });
  const collector = {
...
// node_modules/vitest/dist/vendor/vi.PPwhENHF.js
...
// node_modules/vitest/dist/vendor/index.LgG0iblq.js
...
// src/use-cases/fetch-gyms.spec.ts
var gymsRepository;
var sut;
describe("Fetch Gyms Use Case", () => {
  ...
  it("should be able to fetch all gyms", async () => {
    ...
    globalExpect(gyms).toHaveLength(3);
    globalExpect(gyms).toEqual([
      globalExpect.objectContaining({ id: gym01.id }),
      globalExpect.objectContaining({ id: gym02.id }),
      globalExpect.objectContaining({ id: gym03.id })
    ]);
  });
  ...
  it("should be able to fetch paginated user checkins history", async () => {
    ...
    globalExpect(gyms).toHaveLength(2);
    globalExpect(gyms).toEqual([
      globalExpect.objectContaining({ name: "gym-21" }),
      globalExpect.objectContaining({ name: "gym-22" })
    ]);
  });
});
