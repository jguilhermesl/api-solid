...
var FetchUserCheckInsHistoryUseCase = class {
  constructor(checkinsRepository2) {
...
// src/use-cases/fetch-user-checkins-history.spec.ts
...
describe("Fetch User Checkins History Use Case", () => {
  ...
  it("should be able to fetch user checkins history", async () => {
    ...
    globalExpect(checkins).toEqual([
      globalExpect.objectContaining({ gymId: "gym-01" }),
      globalExpect.objectContaining({ gymId: "gym-02" })
    ]);
  });
  it("should be able to fetch paginated user checkins history", async () => {
    ...
    globalExpect(checkins).toHaveLength(2);
    globalExpect(checkins).toEqual([
      globalExpect.objectContaining({ gymId: "gym-21" }),
      globalExpect.objectContaining({ gymId: "gym-22" })
    ]);
  });
});
